...
- 可选飞书待办提醒，每条有明确时间的计划对应一个个人飞书任务，并固定在开始前 3 小时提醒；
...
打开“设置 -> 飞书待办提醒”后，FeedNote 会以投递记录表所有者的 `union_id` 为负责人，为所有有明确时间且未完成的计划创建飞书任务。任务开始时间和截止时间均取计划时间，提醒固定为提前 180 分钟；计划改期、改名、完成或重新打开会更新同一个任务。缺少时间和已经完成但从未同步过的历史计划不会新建任务。可在 `data\secrets.env` 使用 `FEISHU_TASK_ASSIGNEE_ID` 与 `FEISHU_TASK_ASSIGNEE_ID_TYPE` 显式覆盖自动识别结果。
...
- 飞书待办提醒默认关闭；开启后只向投递记录表所有者同步计划标题、详情、链接、时间、完成状态和固定的提前 3 小时提醒，不读取通讯录或分配给其他用户；
